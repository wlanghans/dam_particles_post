...
! Define Category integer
  call check_nc( nf90_def_var(output_ncid, "Category", nf90_int , dimids , cat_varid) )
  call check_nc( nf90_put_att(output_ncid, cat_varid, "long_name", "Paricle category") )
  call check_nc( nf90_def_var(output_ncid, "N_Atm", nf90_int , dimids , natm_varid) )
  call check_nc( nf90_put_att(output_ncid, natm_varid, "long_name", "Atmos exposures") )
...
! Define Velocity vector
  do i = 1,4
    if (i.le.3) then
      ...
      call check_nc( nf90_def_var(output_ncid, "vt", data_type,dimids,Vel_id(i)) )
    end if
    call check_nc( nf90_put_att(output_ncid, Vel_id(i), "units", Vel_var_unit(i)) )
  enddo
  ...
  call check_nc( nf90_put_att(output_ncid, Vel_id(4), "long_name", "Terminal velocity") )
...
!    call check_nc( nf90_put_att(output_ncid, Scalar_id(i), "_FillValue",-999.) )
  enddo
...
!#if 0
!        read(fread_unit+i) p_info%g_id,p_info%Category,p_info%Pos(:),p_info%Vel(:),p_info%Vterm,p_info%scalar_var(:)
!        Int_Read_Buffer( itmp(1) )  = p_info%g_id
!        Int_Read_Buffer2( itmp(1) ) = p_info%Category
!        do k = 1,3
...
!        enddo
!        R_Read_Buffer( idx+7)     = p_info%Vterm
!        do k = 1,N_Scalar
!          R_Read_Buffer( idx+7+k) = p_info%scalar_var(k)
!        enddo
!#endif
    ...
    do i = 1, Local_Read_in
      ...
      if(idesti /= my_id) then
        ...
        Int_Send_Buffer2( idx )  = Int_Read_Buffer2(i)
        Int_Send_Buffer3( idx )  = Int_Read_Buffer3(i)
      else
        idx = Int_Read_Buffer(i) - id_start + 1
        Category(idx) = Int_Read_Buffer2(i)
        Natm(idx)     = Int_Read_Buffer3(i)
        do j = 1, 3
          Pos(idx,j) = R_Read_Buffer( (i-1)*iskip + j)
          Vel(idx,j) = R_Read_Buffer( (i-1)*iskip + j + 3)
        enddo
        Vel(idx,4) = R_Read_Buffer( (i-1)*iskip + 4 + 3)
        do j = 1, N_Scalar
          Scalar_Var(idx,j) = R_Read_Buffer( (i-1)*iskip + j + 7)
        enddo
      endif
    enddo
...
!WL2013
    ...
    do i = 1, Nproc-1
      idx = N_Sum_Recv(i)
      itmp(1) = N_Recv( mpi_id(i) + 1 )
      call MPI_Irecv(Int_Recv_Buffer2(idx+1:idx+itmp(1)), itmp(1), MPI_INTEGER, mpi_id(i), mpi_id(i), MPI_COMM_WORLD, Recv_Request(i), ierr)

      idx = N_Sum_Send(i)
      itmp(1) = N_Send( mpi_id(i) + 1 )
      call MPI_Isend(Int_Send_Buffer2(idx+1:idx+itmp(1)), itmp(1), MPI_INTEGER, mpi_id(i), my_id, MPI_COMM_WORLD, Send_Request(i), ierr)
    enddo
    ...
    do i = 1, Nproc-1
      idx = N_Sum_Recv(i)
      itmp(1) = N_Recv( mpi_id(i) + 1 )
      call MPI_Irecv(Int_Recv_Buffer3(idx+1:idx+itmp(1)), itmp(1), MPI_INTEGER,mpi_id(i), mpi_id(i), MPI_COMM_WORLD, Recv_Request(i), ierr)

      idx = N_Sum_Send(i)
      itmp(1) = N_Send( mpi_id(i) + 1 )
      call MPI_Isend(Int_Send_Buffer3(idx+1:idx+itmp(1)), itmp(1), MPI_INTEGER,mpi_id(i), my_id, MPI_COMM_WORLD, Send_Request(i), ierr)
    enddo
    ...
    do i = 1, Local_Read_in
      ...
      if(idesti /= my_id) then
        Send_Counter(idesti+1) = Send_Counter(idesti+1) + 1
        if(idesti > my_id ) then
          idx = 4*(N_Sum_Send(idesti  ) + Send_Counter(idesti+1) - 1)
        else
          idx = 4*(N_Sum_Send(idesti+1) + Send_Counter(idesti+1) - 1)
        endif
        R_Send_Buffer( idx+1 ) = R_Read_Buffer( (i-1)*iskip+4 )
        R_Send_Buffer( idx+2 ) = R_Read_Buffer( (i-1)*iskip+5 )
        !if (R_Send_Buffer( idx+2 ).gt.10.0d0) write(*,*) 'v-vel > 10 m/s'
        R_Send_Buffer( idx+3 ) = R_Read_Buffer( (i-1)*iskip+6 )
        R_Send_Buffer( idx+4 ) = R_Read_Buffer( (i-1)*iskip+7 )
        !if (R_Send_Buffer( idx+4 ).lt.-10.0.and.R_Send_Buffer( idx+4 ).ne.-999.0) write(*,*) R_Send_Buffer( idx+4 ), ' v-vel < 10 m/s'
      endif
    enddo

    do i = 1, Nproc-1
      idx = 4*N_Sum_Send(i)
      itmp(1) = 4*N_Send( mpi_id(i) + 1 )
      call MPI_Isend(R_Send_Buffer(idx+1:idx+itmp(1)), itmp(1), MPI_REAL8, mpi_id(i), my_id, MPI_COMM_WORLD, Send_Request(i), ierr)
    enddo
    ...
    do i = 1, N_Sum_Recv(Nproc)
      ...
      Natm(idx)     = Int_Recv_Buffer3(i)
      !if (Category(idx).ne.1) then
      !  write(*,*) 'Category(idx).ne.1; line 401'
      !  call MPI_Finalize(ierr)
      !end if
    ...
    do i = 1, Local_Read_in
      ...
      if(idesti /= my_id) then
        ...
        R_Send_Buffer( idx ) = R_Read_Buffer( (i-1)*iskip+8 )
      endif
    enddo
    ...
    do i = 1, N_Sum_Recv(Nproc)
      ...
      Vel(idx,2) = R_Recv_Buffer( (i-1)*4 + 2)
      Vel(idx,3) = R_Recv_Buffer( (i-1)*4 + 3)
      Vel(idx,4) = R_Recv_Buffer( (i-1)*4 + 4)
      if (Vel(idx,4).gt.0.0 .or. Vel(idx,4).lt.-15.) write(*,*) 'Vterm ', Vel(idx,4)
    enddo
    ...
    do k = 1, N_Scalar - 1
      ...
      do i = 1, Local_Read_in
        ...
        if(idesti /= my_id) then
          ...
          R_Send_Buffer( idx ) = R_Read_Buffer( (i-1)*iskip+8+k )
        endif
      enddo
    ...
    call check_nc( nf90_put_var(output_ncid, cat_varid, Category(:), start=(/1,step/),count=(/Np_local,1/)) )
    call check_nc( nf90_put_var(output_ncid, natm_varid,    Natm(:), start=(/1,step/),count=(/Np_local,1/)) )
    do k = 1,3
    ...
    enddo
      call check_nc( nf90_put_var(output_ncid, Vel_id(4), real(Vel(:,4),data_bytes), start=(/1,step/),count=(/Np_local,1/)) )
    do k = 1,N_Scalar
...
!      call check_nc( nf90_put_att(restart_ncid, Restart_Var_id(i+6), "_FillValue",-999.) )
    enddo
